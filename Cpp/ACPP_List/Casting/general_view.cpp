...
class Dog
{
	...
	friend std::ostream& operator<<(std::ostream &os, Dog &dog) {
		os << "This is a dog named: " << dog.m_name << std::endl;
		return os;
	}
public:
	Dog(std::string name) :
	...
	virtual void iAm() { std::cout << "I'm a Dog named: " << this->m_name << std::endl; }
	

	~Dog()
	{
		std::cout << "Dog " << this->m_name << ", destroyed" << std::endl;
	}
...
public:
	...
	~YellowDog()
	{
		std::cout << "YellowDog " << this->m_name << ", Destroyed" << std::endl;
	}
};
